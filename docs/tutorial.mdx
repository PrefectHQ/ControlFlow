...
ControlFlow has a shortcut for creating tasks from Python functions: the `@task` decorator. This allows you to define tasks in a more familiar, Pythonic way. Here's how we could rewrite the previous example using the `@task` decorator:
...
Both of these conveniences are because `@task`-decorated functions are executed eagerly by default. You can recover lazy execution by passing `lazy=True` to the `@task` decorator, in which case a `Task` object will be returned when you call the function.

In complex workflows, lazy execution may be preferred, as it allows ControlFlow to optimize task execution and agent selection based on observed dependencies. However, eager execution can be useful for testing or interactive use, as it behaves more like a normal Python function.
