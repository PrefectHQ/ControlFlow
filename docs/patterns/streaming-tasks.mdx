...
- `Stream.TASK_EVENTS`: Task state change events
...
### Workflow Events
These events mark key points in task execution:
...
For streaming cases, the delta events (e.g. AgentMessageDelta, AgentContentDelta) provide incremental updates as the LLM generates its response.
