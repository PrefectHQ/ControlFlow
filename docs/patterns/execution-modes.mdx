...
Eager mode is the default for the [functional API](/patterns/workflow-apis). In this mode, flows and tasks are executed immediately when called.
...
Lazy execution is the only mode available for the [imperative API](/patterns/workflow-apis), as imperative tasks must be run explicitly. You can also run functional tasks lazily by passing `lazy_=True` when calling the task.
