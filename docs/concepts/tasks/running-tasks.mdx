...
A task describes an objective for one or more agents to accomplish. When a task is run, an orchestrator object coordinates all assigned agents to complete the task. 

There are a few ways to [create tasks](/concepts/tasks/creating-tasks), and the method you choose will determine how you run the task.

## Running a `Task` object

To run a `Task` object, call its `run()` method.
