...
We can also use the `Moderated` strategy to use a more powerful model to orchestrate smaller ones:

```python Moderated
import controlflow as cf
from langchain_openai import ChatOpenAI

agent1 = cf.Agent(name="Agent 1", model=ChatOpenAI(model="gpt-4o-mini"))
agent2 = cf.Agent(name="Agent 2", model=ChatOpenAI(model="gpt-4o-mini"))
